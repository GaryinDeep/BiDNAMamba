...
from lib.model.hyenadna.config_hyenadna import hyenadnaConfig
from lib.model.hyenadna.utils.hf import load_config_hf, load_state_dict_hf
...
class HyenaDNAModel(nn.Module):
    ...
    def __init__(
        ...
        # check if layer (config) has d_model (HF code differs from main Safari code)
        if layer is None:
            layer = {"d_model": d_model, "l_max": seq_len_max}
        elif 'd_model' not in layer:
            layer['d_model'] = d_model
        elif "l_max" not in layer:
            layer['l_max'] = seq_len_max
    ...
    def forward(self, input_ids, position_ids=None, state=None, num_last_tokens=0): 
        hidden_states = self.backbone(input_ids, position_ids=position_ids) # (n_batch, l_seq, d_model)
    ...
    def from_pretrained(cls, pretrained_model_name, device=None, dtype=None, **kwargs): 
        ...
        config = hyenadnaConfig(**config_data)
...
用于自监督微调
"""
class HyenaDNAModel_sft(HyenaDNAModel):
